            ...
            return (
                <li className="thread-item collection-item avatar" id={thread.id} key={thread.id}>
                    ...
                    <span className="title">
						...
						<Link to={{ pathname: `/forum/${this.props.match.params.forumcatid}/${this.props.match.params.forumsubcatid}/${thread.subject}?threadid=${thread.id}`, state: { threadid: thread.id} }}>
							{thread.subject}
						</Link>
                    </span>
                    {Parser(thread.text)}
